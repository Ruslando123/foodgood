...
type Consent = {
  ...
  terms: { accepted: boolean; version: string | null; acceptedAt: string | null; currentVersion: string };
...
export default function ConsentPreferences() {
  ...
  const termsAcceptedDate = consent?.terms.acceptedAt ? new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium" }).format(new Date(consent.terms.acceptedAt)) : null;
  return <section className="overflow-hidden rounded-[17px] border border-black/[0.08] bg-white">
    ...
    <div className="border-t border-black/[0.07] px-4 py-3"><p className="text-sm font-medium">Условия использования</p><p className="mt-0.5 text-xs text-muted">Отдельный документ · версия {consent?.terms.currentVersion ?? "…"}</p>{consent?.terms.accepted ? <p className="mt-1 text-xs font-medium text-primary">Приняты {termsAcceptedDate ?? ""}</p> : consent ? <button type="button" disabled={busy} onClick={() => void save({ acceptTerms: true })} className="mt-2 min-h-11 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">Принять условия</button> : null}<Link href="/legal/terms" className="mt-2 block text-xs font-semibold text-primary underline">Открыть условия использования</Link></div>
