...
export default function BusinessBagActions({ id, quantityLeft, editable }: { id: string; quantityLeft: number; editable: boolean }) {
  ...
  return <div className="mt-3 min-w-0 space-y-3 border-t pt-3">
    ...
    <button disabled={busy} onClick={repeat} className="min-h-11 w-full rounded-xl border border-primary px-3 py-2.5 text-sm font-semibold text-primary disabled:opacity-40">{busy ? "Обновляем…" : "Повторить этот набор завтра"}</button>
