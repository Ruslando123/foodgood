...
export default async function AdminOverviewPage() {
  const [venues, owners, customers, activeBags, activeOrders, revenue, recentOrders, recentVenues] = await Promise.all([
    prisma.venue.count(),
  ...
  const stats = [
    { label: "Оборот заведений", value: price(gross), hint: "FoodGood не принимает деньги", href: "/admin/orders?status=COMPLETED" },
    ...
    { label: "Покупатели", value: customers, hint: "Клиентская база", href: "/admin/customers" },
    { label: "Владельцы", value: owners, hint: "Партнёрская база", href: "/admin/owners" },
  ];
  ...
  return <main className="mx-auto space-y-6 overflow-hidden p-4 sm:p-6" style={{ width: "100vw", maxWidth: "72rem", overflowWrap: "anywhere" }}>
    <div><h1 className="text-2xl font-bold">Обзор платформы</h1><p className="mt-1 text-sm text-muted">Брони, заведения, пользователи и рост клиентской базы.</p></div>
