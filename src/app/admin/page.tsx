...
export default async function AdminOverviewPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{stats.map((stat) => <Link key={stat.label} href={stat.href} className="rounded-2xl border border-black/[0.08] bg-white p-4 transition hover:border-primary/30"><p className="text-xs text-muted">{stat.label}</p><p className="mt-1 text-2xl font-bold text-primary">{stat.value}</p><p className="mt-1 text-xs text-muted">{stat.hint}</p></Link>)}</section>
    <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr]">
      <section className="overflow-hidden rounded-2xl border border-black/[0.08] bg-white"><div className="flex items-center justify-between border-b border-black/[0.08] p-4"><div><h2 className="font-semibold">Последние заказы</h2><p className="text-xs text-muted">Недавняя активность покупателей</p></div><Link href="/admin/orders" className="text-sm font-semibold text-primary">Все заказы</Link></div>{recentOrders.length === 0 ? <p className="p-5 text-sm text-muted">Заказов пока нет.</p> : <div className="divide-y divide-black/[0.07]">{recentOrders.map((order) => <Link key={order.id} href={`/admin/orders?q=${order.id}`} className="grid gap-2 p-4 hover:bg-black/[0.02] sm:grid-cols-[1fr_auto]"><div className="min-w-0"><p className="truncate text-sm font-semibold">{order.bag.venue.name} · {order.bag.title}</p><p className="truncate text-xs text-muted">{order.user.phone ?? order.user.name ?? "Покупатель"} · {order.quantity} шт.</p></div><div className="sm:text-right"><p className="text-sm font-semibold">{price(order.totalPrice)}</p><p className="text-xs text-muted">{ORDER_LABELS[order.status] ?? order.status}</p></div></Link>)}</div>}</section>
      <section className="overflow-hidden rounded-2xl border border-black/[0.08] bg-white"><div className="flex items-center justify-between border-b border-black/[0.08] p-4"><div><h2 className="font-semibold">Новые заведения</h2><p className="text-xs text-muted">Последние регистрации</p></div><Link href="/admin/venues" className="text-sm font-semibold text-primary">Все</Link></div>{recentVenues.length === 0 ? <p className="p-5 text-sm text-muted">Заведений пока нет.</p> : <div className="divide-y divide-black/[0.07]">{recentVenues.map((venue) => <Link key={venue.id} href={`/admin/venues/${venue.id}`} className="flex items-center justify-between gap-3 p-4 hover:bg-black/[0.02]"><div className="min-w-0"><p className="truncate text-sm font-semibold">{venue.name}</p><p className="truncate text-xs text-muted">{venue.owner.name ?? venue.owner.phone ?? "Без владельца"}</p></div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${venue.status === "ACTIVE" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}>{venue.status === "ACTIVE" ? "Активно" : "Пауза"}</span></Link>)}</div>}</section>
    </div>
  </main>;
}
