...
export default function AdminVenuesPage() {
  ...
  const [venues, setVenues] = useState<Venue[]>([]); const [query, setQuery] = useState(""); const [status, setStatus] = useState(searchParams.get("status") === "SUSPENDED" ? "SUSPENDED" : "ALL"); const [page, setPage] = useState(1); const [error, setError] = useState<string | null>(null);
  const load = useCallback(async () => { try { setVenues((await api<{ venues: Venue[] }>("/api/admin/venues")).venues); } catch (e) { setError(e instanceof Error ? e.message : "Не удалось загрузить заведения"); } }, []);
  useEffect(() => { load(); }, [load]);
  const filtered = useMemo(() => venues.filter((venue) => (status === "ALL" || venue.status === status) && `${venue.name} ${venue.address} ${venue.owner.phone ?? ""} ${venue.owner.name ?? ""}`.toLowerCase().includes(query.toLowerCase())), [venues, query, status]);
