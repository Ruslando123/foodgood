...
export type EditableBusinessVenue = {
  id: string;
  name: string;
  description: string;
  address: string;
...
type Form = EditableBusinessVenue;
const field = "mt-1 w-full rounded-xl border px-3 py-2.5 outline-none focus:border-primary";

export default function EditBusinessVenueClient({ initialVenue }: { initialVenue: EditableBusinessVenue }) {
  const id = initialVenue.id; const router = useRouter();
  const [form, setForm] = useState<Form>(initialVenue); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
