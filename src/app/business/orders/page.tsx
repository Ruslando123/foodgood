...
const LABELS: Record<string, string> = { RESERVED: "Забронирован", READY_FOR_PICKUP: "Готов к выдаче", COMPLETED: "Выдан", CANCELLED: "Отменён", EXPIRED: "Истёк" };
...
export default async function BusinessOrdersPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6">
    <header><h1 className="text-2xl font-bold">Брони</h1><p className="mt-1 text-sm text-muted">Отметьте заказ готовым и выдайте его после оплаты на кассе.</p></header>
    <section className="grid grid-cols-2 gap-3 sm:grid-cols-5"><Link href="/business/orders" className="rounded-xl border bg-white p-3"><p className="text-xs text-muted">Все</p><p className="text-xl font-bold">{total}</p></Link>{["RESERVED", "READY_FOR_PICKUP", "COMPLETED", "CANCELLED"].map((item) => <Link key={item} href={`/business/orders?status=${item}`} className="rounded-xl border bg-white p-3"><p className="truncate text-xs text-muted">{LABELS[item]}</p><p className="text-xl font-bold">{counts[item] ?? 0}</p></Link>)}</section>
    ...
    <section className="overflow-hidden rounded-2xl border bg-white">{orders.length === 0 ? <p className="p-8 text-center text-sm text-muted">Брони не найдены.</p> : <div className="divide-y">{orders.map((order) => <Link key={order.id} href={`/business/orders/${order.id}`} className="grid gap-4 p-4 hover:bg-black/[0.02] md:grid-cols-[1.3fr_1fr_auto]"><div><p className="font-semibold">{order.bag.title} · {order.quantity} шт.</p><p className="text-xs text-muted">{order.bag.venue.name} · {new Date(order.createdAt).toLocaleString("ru-RU")}</p><p className="mt-1 text-xs text-muted">{order.user.phone ?? order.user.name ?? "Покупатель"}</p></div><div><p className="font-mono text-lg font-bold tracking-widest">{order.pickupCode}</p><p className="text-sm font-semibold">{price(order.totalPrice)}</p><p className="text-xs text-muted">Оплата при получении</p></div><div className="md:text-right"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${statusClass(order.status)}`}>{LABELS[order.status] ?? order.status}</span><p className="mt-2 text-xs font-semibold text-primary">Открыть →</p></div></Link>)}</div>}</section>
  </main>;
}
