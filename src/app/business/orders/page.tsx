...
export default async function BusinessOrdersPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6">
    ...
    <section className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Link href="/business/orders" className="rounded-xl border bg-white p-3"><p className="text-xs text-muted">Все</p><p className="text-xl font-bold">{total}</p></Link>{["RESERVED", "READY_FOR_PICKUP", "COMPLETED", "CANCELLED_BY_PARTNER", "NO_SHOW", "DISPUTED"].map((item) => <Link key={item} href={`/business/orders?status=${item}`} className="rounded-xl border bg-white p-3"><p className="truncate text-xs text-muted">{LABELS[item]}</p><p className="text-xl font-bold">{counts[item] ?? 0}</p></Link>)}</section>
