...
try {
  ...
  expectConstraintViolation("Order_support_category_check", `UPDATE "Order" SET "supportStatus" = 'OPEN', "supportCategory" = 'INVALID', "supportOpenedAt" = now() WHERE id = 'migration-order'`);
  expectConstraintViolation("Order_support_timestamps_check", `UPDATE "Order" SET "supportStatus" = 'RESOLVED', "supportCategory" = 'OTHER', "supportResolvedAt" = now() WHERE id = 'migration-order'`);
  ...
  console.log("Upgrade migration preserved data, validated CHECK constraints, rejected invalid rows, and left schema diff empty.");
} finally {
